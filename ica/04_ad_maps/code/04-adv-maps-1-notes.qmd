...
Lower than the one that we have 
...
```{r}
#Transforming the CRS the county data to the more local CRS of the cities
mn_counties <- mn_counties |>
    st_transform(crs = st_crs(mn_cities))
```

```{r}
#Checking the new CRS for mn_counties
st_crs(mn_counties)
```
...
ggplot() + 
    ...
    geom_sf(data = mn_cities, size = 0.4) + 
    ggthemes::theme_map()
```
...
It is more accessible for those who have visual impediments 

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

_d() is used when color is put to a discrete (categorical) variable.
_c() is used when color is put to a continuous variable.
_b() is used when color is put to a continuous variable. However, only if we want the continuous variable to be set so that there is a small set of colors.
...
ggplot() +
    ...
    labs(title = "Minnesota Cities with Population over 10,000") + 
    ggthemes::theme_map() +
