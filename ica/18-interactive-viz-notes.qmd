...
1.  Catalog the app's **layout** and **interactivity features** and make note of where user inputs are located and how different parts of the app respond to user input.
    ...
    The app is organized into a put together dashboard featuring a persistent left-hand sidebar for global inputs, which dynamically update the visual data across three specialized tabs.
    While the "Explore metros" tab facilitates a deep dive into 2020 data through interactive scatterplots and maps that reveal specific neighborhood demographics upon a user's click, the "Compare over time" and "About" sections provide temporal analysis and technical methodology, respectively.
    This interconnected layout ensures that user selections in the sidebar or direct interactions with the charts immediately filter and refresh the spatial visualizations to show evolving diversity patterns.
...
2.  Evaluate the design of this app.
    ...
    -   What explorations/comparisons are you curious to explore that are not enabled well by the app?
        ...
        The application’s interactivity is essential because it allows for "spatial drilling," enabling users to instantly connect abstract statistical outliers on a scatterplot to physical neighborhoods on a map, a process that would be cumbersome with static images.
        However, it lacks the ability to perform direct side-by-side benchmarking between different cities or to overlay socioeconomic factors like transit and school districts that often drive these demographic shifts.
...
```{r}
rsconnect::setAccountInfo(name='pescobar029',
			  token='0BB64935F83F208D8BCD3D3CE003DC71',
			  secret='I0NTIDnUkiIakZjnSj80xbPGdshEsaF0RQ7+4iWG')
```
